...
function getDataDir(username) {
    const dir = path.join(__dirname, "data", username);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    return dir;
}

function loadConfig(username = "legacy_user") {
    try {
        const configPath = path.join(getDataDir(username), "config.json");
        if (fs.existsSync(configPath)) {
            return JSON.parse(fs.readFileSync(configPath, "utf8"));
        }
    } catch (err) {
        console.error(`Error reading config.json for ${username}, using fallback defaults`, err);
    }
    return {
...
function parseBalance(text) {
    if (!text.startsWith("ACCESS_BALANCE:")) return null;
    try {
...
async function getPriceForService(config, service, country, operator) {
    ...
    if (typeof serviceBlock !== "object" || serviceBlock === null) return null;
...
async function getOtp(requestId, timeoutSeconds = 300, pollInterval = 2, config) {
    ...
    while (true) {
        ...
        if ((status === "ok" || status === "cancelled") && otp) return otp;
        if (Date.now() >= deadline) return otp; 
...
function parseCancelStatus(text) {
    if (text.startsWith("ACCESS_CANCEL")) return "accepted";
    if (text.startsWith("ACCESS_CANCEL_ALREADY")) return "already_cancelled";
    return text;
}
...
function parseOtpResponse(text) {
    if (text.startsWith("STATUS_OK:")) return { status: "ok", otp: extractOtp(text) };
    if (text.startsWith("STATUS_CANCEL")) return { status: "cancelled", otp: null };
    if (text.startsWith("ACCESS_WAITING")) return { status: "waiting", otp: null };
    return { status: "unknown", otp: extractOtp(text) };
}

async function requestNewOtpUntilNew(requestId, previousOtp = null, timeoutSeconds = 300, pollInterval = 2, config) {
    const cfg = config || loadConfig();
    const deadline = Date.now() + timeoutSeconds * 1000;
...
async function sendOtp(mobileNo, config, log = console.log) {
    const cfg = config || loadConfig();
    try {
        const response = await axios.post(`${cfg.TARGET_BASE_URL}/sendSmsCode`, { mobileNo }, { headers: { "Content-Type": "application/json" }});
...
async function registerUser(mobileNo, password, smsCode, inviteCode, config, log = console.log) {
    const cfg = config || loadConfig();
    try {
        const response = await axios.post(`${cfg.TARGET_BASE_URL}/register`, { mobileNo, password, smsCode, inviteCode }, { headers: { "Content-Type": "application/json" }});
...
function saveFailedRegistration(username, phoneNumber, password, smsCode, errorMessage, log = console.log) {
    const filePath = path.join(getDataDir(username), "failed_registrations.json");
    let failures = [];
    try {
        if (fs.existsSync(filePath)) failures = JSON.parse(fs.readFileSync(filePath, "utf8"));
    } catch (e) {
        log(`Failed to read failed_registrations.json: ${e.message}`);
    }

    failures.push({
        phoneNumber, password, smsCode, error: errorMessage, createdAt: new Date().toISOString()
    });
...
async function runSingleCycle(username, config, log = console.log) {
    const cfg = config || loadConfig(username);
    let requestId = null;
    ...
    try {
        ...
        if (!numResult) throw new Error("Failed to request virtual number.");
        ...
        if (!otpSent) throw new Error("Failed to trigger OTP send in the application.");

        log("Polling for OTP (30 seconds timeout)...");
        smsCode = await getOtp(requestId, 30, 2, cfg);
        if (!smsCode) throw new Error("Failed to retrieve OTP within timeout limit.");
        
        log(`Successfully retrieved OTP: ${smsCode}`);
        log("Registering user with target application...");
        
        const token = await registerUser(phoneNumber, cfg.PASSWORD, smsCode, cfg.INVITE_CODE, cfg, log);
        const accountRecord = { phoneNumber, password: cfg.PASSWORD, smsCode, jwtToken: token, createdAt: new Date().toISOString() };

        const accountsPath = path.join(getDataDir(username), "accounts.json");
        let accounts = [];
        try {
            if (fs.existsSync(accountsPath)) accounts = JSON.parse(fs.readFileSync(accountsPath, "utf8"));
        } catch (e) { log(`Failed to read accounts.json: ${e.message}`); }
        ...
        } catch (e) { log(`Failed to write to accounts.json: ${e.message}`); }
        ...
        if (phoneNumber && smsCode) {
            ...
            saveFailedRegistration(username, phoneNumber, cfg.PASSWORD, smsCode, err.message, log);
        }
        if (requestId) {
            ...
            } catch (cancelErr) { log(`Cancel error: ${cancelErr.message}`); }
        }
...
async function main() {
    ...
    const username = usernameIndex !== -1 ? args[usernameIndex + 1] : "legacy_user";
    
    const result = await runSingleCycle(username);
    if (result.success) {
